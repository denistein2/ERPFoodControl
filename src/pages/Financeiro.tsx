...
import { TrendingUp, AlertTriangle, Clock, Banknote, ArrowUpRight, ArrowDownRight, PlusCircle, Settings as SettingsIcon, Save } from 'lucide-react';
...
type TituloStatus = 'aberto' | 'parcial' | 'quitado' | 'cancelado';

interface BaseTitulo {
  id: number;
  numero: string;
  tipo: string;
...
interface TituloReceber extends BaseTitulo {
  cliente: string;
}

interface TituloPagar extends BaseTitulo {
  fornecedor: string;
}

type Titulo = TituloReceber | TituloPagar;
...
const mockReceber: TituloReceber[] = [
...
function TitulosTab({ titulos, entityLabel }: { titulos: Titulo[]; entityLabel: string }) {
              ...
              {sorted.map(t => {
                ...
                return (
                  <TableRow key={t.id} className={vencido ? 'bg-destructive/5' : ''}>
                    ...
                    <TableCell>{'cliente' in t ? t.cliente : t.fornecedor}</TableCell>
...
function FluxoDeCaixaTab() {
  ...
  const [movements, setMovements] = useState<CashierMovement[]>([]);
  const [loading, setLoading] = useState(true);
  const [newMovement, setNewMovement] = useState<{ description: string; amount: string; type: MovementType }>({ description: '', amount: '', type: 'Entrada' });
  ...
  const fetchData = async () => {
    ...
    if (session) {
      ...
      const allMovements: CashierMovement[] = [
        ...((mv || []) as Omit<CashierMovement, 'isManual'>[]).map((movement) => ({ ...movement, isManual: true })),
        ...((txs || []) as FinancialTransaction[]).map((transaction) => {
          const paymentMethod = Array.isArray(transaction.payment_methods)
          ...
          return {
            ...
            method: paymentMethod?.name || 'Venda',
...
function PixSettingsTab() {
  const [config, setConfig] = useState<PixConfig>({ pix_key: '', merchant_name: '', city: '' });
  const [loading, setLoading] = useState(true);
  ...
  const handleSave = async () => {
    ...
    const { error } = config.id 
      ? await supabase.from('pix_config').update(payload).eq('id', config.id)
      : await supabase.from('pix_config').insert([payload]);

    if (error) {
      toast.error('Erro ao salvar configurações PIX');
    } else {
      toast.success('Configurações PIX salvas!');
      // Recarregar para pegar o ID se for novo
      const { data } = await supabase.from('pix_config').select('*').maybeSingle();
      if (data) setConfig(data);
    }
  };
      ...
      <CardContent className="space-y-4 max-w-md">
        ...
        <div className="space-y-2">
          <Label className="flex justify-between">
            ...
            <span className="text-[10px] text-muted-foreground">{config.merchant_name.length}/25</span>
          </Label>
          <Input 
            placeholder="Ex: Stein Technology" 
            maxLength={25}
            value={config.merchant_name}
            onChange={e => setConfig(p => ({ ...p, merchant_name: e.target.value }))}
          />
          <p className="text-[10px] text-muted-foreground italic">Nome exatamente como cadastrado no seu banco.</p>
        </div>
        <div className="space-y-2">
          <Label className="flex justify-between">
            <span>Cidade</span>
            <span className="text-[10px] text-muted-foreground">{config.city.length}/15</span>
          </Label>
          <Input 
        ...
        <Button 
          ...
          onClick={handleSave}
          disabled={!config.pix_key || !config.merchant_name || !config.city}
        >
          <Save className="mr-2 h-4 w-4" /> Salvar Configurações PIX
        </Button>
      </CardContent>
    </Card>
  );
    ...
    <div className="space-y-6">
        ...
        <Tabs defaultValue="caixa">
          <TabsList>
            ...
            <TabsTrigger value="config">Configurações PIX</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="config"><PixSettingsTab /></TabsContent>
        </Tabs>
      </div>
  );
}
