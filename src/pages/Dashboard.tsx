...
interface FinancialTransactionWithPayment {
  id: string;
  amount: number;
  created_at: string;
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (batchesData) setBatches(batchesData as BakingBatch[]);
      ...
      if (recentData) {
        setRecentSales(
          (recentData as FinancialTransactionWithPayment[]).map((sale) => {
            const paymentMethod = Array.isArray(sale.payment_methods)
              ? sale.payment_methods[0]
